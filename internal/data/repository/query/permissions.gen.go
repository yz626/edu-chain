...
func newPermission(db *gorm.DB, opts ...gen.DOOption) permission {
	...
	_permission.PathPattern = field.NewString(tableName, "path_pattern")
	_permission.Method = field.NewString(tableName, "method")
	_permission.IsAPI = field.NewBool(tableName, "is_api")
	_permission.IsMenu = field.NewBool(tableName, "is_menu")
	_permission.MenuIcon = field.NewString(tableName, "menu_icon")
	_permission.MenuOrder = field.NewInt64(tableName, "menu_order")
	_permission.ParentID = field.NewString(tableName, "parent_id")
	_permission.Level = field.NewInt64(tableName, "level")
	_permission.IsActive = field.NewBool(tableName, "is_active")
	_permission.ExtraData = field.NewString(tableName, "extra_data")
...
type permission struct {
	...
	ALL         field.Asterisk
	ID          field.String // 权限ID (UUID)
	Code        field.String // 权限代码
	Name        field.String // 权限名称
	Resource    field.String // 资源类型
	Action      field.String // 操作类型
	Description field.String // 权限描述
	PathPattern field.String // API路径匹配模式
	Method      field.String // HTTP方法
	IsAPI       field.Bool   // 是否为API权限
	IsMenu      field.Bool   // 是否菜单权限
	MenuIcon    field.String // 菜单图标
	MenuOrder   field.Int64  // 菜单排序
	ParentID    field.String // 父级权限ID
	Level       field.Int64  // 权限级别
...
func (p *permission) updateTableName(table string) *permission {
	...
	p.PathPattern = field.NewString(table, "path_pattern")
	p.Method = field.NewString(table, "method")
	p.IsAPI = field.NewBool(table, "is_api")
	p.IsMenu = field.NewBool(table, "is_menu")
	p.MenuIcon = field.NewString(table, "menu_icon")
	p.MenuOrder = field.NewInt64(table, "menu_order")
	p.ParentID = field.NewString(table, "parent_id")
	p.Level = field.NewInt64(table, "level")
	p.IsActive = field.NewBool(table, "is_active")
	p.ExtraData = field.NewString(table, "extra_data")
...
func (p *permission) fillFieldMap() {
	...
	p.fieldMap["path_pattern"] = p.PathPattern
	p.fieldMap["method"] = p.Method
	p.fieldMap["is_api"] = p.IsAPI
	p.fieldMap["is_menu"] = p.IsMenu
	p.fieldMap["menu_icon"] = p.MenuIcon
	p.fieldMap["menu_order"] = p.MenuOrder
	p.fieldMap["parent_id"] = p.ParentID
	p.fieldMap["level"] = p.Level
	p.fieldMap["is_active"] = p.IsActive
	p.fieldMap["extra_data"] = p.ExtraData
